...
const Button: React.FC<React.PropsWithChildren<IProps>> = ({
  ...
  return (
    <button
      onClick={onClick}
      className={`relative inline-flex items-center rounded-md px-3 py-2 text-sm font-semibold shadow-sm focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 ${VARIANT_VALUES[variant]} ${className || ""}`}
      {...props}
    >
      {children}
    </button>
